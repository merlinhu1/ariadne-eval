...
Ariadne Eval V1 is a local, state.db-ingested instruction-health evaluator for Hermes Agent sessions. It includes an LLM judge for final ratings, but does not require a Hermes plugin.
...
```text
Hermes state.db -> reader -> normalizer -> signal extractor -> preflight trim/prefilter -> LLM judge -> evals.db -> CLI
                              |                                   ^
                              +---------- persisted evidence -----+
```
...
Hermes already stores enough durable session data for the first useful evaluator. Removing the plugin, scheduler, dashboard, and generic adapters from V1 keeps installation and debugging simple. Keeping the LLM judge preserves the core product value: rating each turn's health.
