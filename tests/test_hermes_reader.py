...
def make_state_db(home: Path) -> Path:
    ...
    with closing(sqlite3.connect(db_path)) as con:
        ...
        CREATE TABLE sessions (
            ...
            message_count INTEGER DEFAULT 0, tool_interaction_count INTEGER DEFAULT 0,
            input_tokens INTEGER DEFAULT 0, output_tokens INTEGER DEFAULT 0,
            reasoning_tokens INTEGER DEFAULT 0, estimated_cost_usd REAL, actual_cost_usd REAL,
            title TEXT, source_session_api_interaction_count INTEGER DEFAULT 0
        );
        ...
        INSERT INTO sessions (id, source, user_id, model, started_at, ended_at, message_count, tool_interaction_count, input_tokens, output_tokens, title, source_session_api_interaction_count)
...
class HermesStateReaderTest(unittest.TestCase):
    def test_reads_recent_sessions_and_orders_messages_without_reasoning_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(sessions[0]["tool_interaction_count"], 1)
