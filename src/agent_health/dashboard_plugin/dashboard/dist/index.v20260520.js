(function () {
  ...
  function taskFormDefaults(task) {
    ...
    return {
      ...
      interval_minutes: Math.max(1, Math.round(Number(source.interval_seconds || 3600) / 60)),
  ...
  function taskPayloadFromForm(form) {
    return {
      ...
      interval_seconds: Math.max(1, Number(form.interval_minutes || 1)) * 60,
      ...
      h("div", { className: "ae-config-grid" },
        ...
        h("form", { className: "ae-config-card ae-config-task", onSubmit: saveTask },
          ...
          h("div", { className: "ae-config-form-grid" },
            ...
            h(Field, { label: "Interval minutes" }, h("input", { type: "number", min: "1", value: form.interval_minutes, onChange: (ev) => updateForm("interval_minutes", ev.target.value) })),
