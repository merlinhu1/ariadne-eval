...
- V1 may trigger judging through an explicit CLI batch command, `agent-health eval --due`, or through explicitly configured recurring eval tasks processed by `agent-health scheduler tick` / `agent-health scheduler run --poll-seconds N`.
...
- Keep default judge batches budget-safe: score due candidates with deterministic signals, including specific rule/model incident categories when available; skip low-priority candidates by default; trim bulky documents/code/images/tool previews before provider calls; cap judge calls per invocation; skip any previously judged unit unless `--reevaluate` is explicit; defer no-reaction last turns behind a cooldown; and record provider-reported judge token usage.
...
- Deterministic signals and tiny non-LLM classifier outputs are evidence and fallback inspection data, not replacements for the judge; local models are expected to leave some semantic cases below high-confidence coverage.
